...
class ClientesController extends AdminController
{
	...
	public function duplicate()
	{
		...
		try
		{
			if (empty($pks))
			{
				throw new \Exception(Text::_('COM_ICODE_CLIENTES_NO_ELEMENT_SELECTED'));
			}

			ArrayHelper::toInteger($pks);
			$model = $this->getModel();
			$model->duplicate($pks);
			$this->setMessage(Text::_('COM_ICODE_CLIENTES_ITEMS_SUCCESS_DUPLICATED'));
		}
		catch (\Exception $e)
		{
		...
		$this->setRedirect('index.php?option=com_icode_clientes&view=clientes');
	}
	 ...
	 * @since   1.0.0
	 */
	public function getModel($name = 'Cliente', $prefix = 'Administrator', $config = array())
	{
