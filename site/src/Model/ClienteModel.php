...
/**
 * Icode_clientes model.
 *
 * @since  1.0.0
 */
class ClienteModel extends ItemModel
{
	public $_item;
	...
	{
		$app  = Factory::getApplication('com_icode_clientes');
		$user = $app->getIdentity();

		// Check published state
		if ((!$user->authorise('core.edit.state', 'com_icode_clientes')) && (!$user->authorise('core.edit', 'com_icode_clientes')))
		{
		...
		{
			$id = Factory::getApplication()->getUserState('com_icode_clientes.edit.cliente.id');
		}
		else
		{
			$id = Factory::getApplication()->input->get('id');
			Factory::getApplication()->setUserState('com_icode_clientes.edit.cliente.id', $id);
		}
		...
		if ($this->_item === null)
		{
			...
			if ($table && $table->load($id))
			{
				...
				if ($published = $this->getState('filter.published'))
				{
					...
					{
						throw new \Exception(Text::_('COM_ICODE_CLIENTES_ITEM_NOT_LOADED'), 403);
					}
				}
			...
			if (empty($this->_item))
			{
				throw new \Exception(Text::_('COM_ICODE_CLIENTES_ITEM_NOT_LOADED'), 404);
			}
		}
	 ...
	 */
	public function getTable($type = 'Cliente', $prefix = 'Administrator', $config = array())
	{
	...
	public function getItemIdByAlias($alias)
	{
		...
		{
			$aliasKey   = $this->getAliasFieldNameByView('cliente');
		}
